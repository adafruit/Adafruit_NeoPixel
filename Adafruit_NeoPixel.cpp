...
Adafruit_NeoPixel::Adafruit_NeoPixel(uint16_t n, uint8_t p, uint8_t t) {
    ...
    if((pixels = (uint8_t *)malloc(numBytes))) {
        ...
        type    = t;
        pin     = p;
#ifndef __SAM3X8E__ // needs to be disabled for the Arduino Due
        port    = portOutputRegister(digitalPinToPort(p));
        pinMask = digitalPinToBitMask(p);
#endif  // end of needs to be disabled for the Arduino Due
        endTime = 0L;
    } else {
        numLEDs = 0;
    }
...
#ifdef __arm__
//this is for setting the pins of the arduino due fast
...
// cli() and sei() is not supported by the Due, therefore
// everything is encapsulate asking for the due. This way as much
// as possible original code stays untouched.
